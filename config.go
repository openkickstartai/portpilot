...
func validateConfig(cfg *Config) error {
	...
	if cfg.ReconnectDelay != "" {
		if _, err := time.ParseDuration(cfg.ReconnectDelay); err != nil {
			return fmt.Errorf("invalid reconnect_delay format: %w", err)
		}
	}
...
func validateTunnel(name string, tunnel TunnelConfig) error {
	...
	if tunnel.KeyFile != "" {
		...
		if _, err := os.Stat(tunnel.KeyFile); os.IsNotExist(err) {
			return fmt.Errorf("key file does not exist: %s", tunnel.KeyFile)
		}
	}
