...
func validateConfig(cfg *Config) error {
	...
	for name, tunnel := range cfg.Tunnels {
		if err := validateTunnel(name, tunnel); err != nil {
			return fmt.Errorf("tunnel '%s' validation failed: %w", name, err)
		}
	}
...
func validateTunnel(name string, tunnel TunnelConfig) error {
	...
	if tunnel.KeyFile != "" {
		...
		if info, err := os.Stat(cleanKeyPath); err == nil {
			mode := info.Mode().Perm()
			if mode&0044 != 0 {
				log.Printf("Warning: key file %s has overly permissive permissions %o", cleanKeyPath, mode)
			}
		}
	}
	
	log.Printf("Validated tunnel '%s': %s@%s:%d -> %s:%d", name, tunnel.User, tunnel.Host, tunnel.LocalPort, tunnel.RemoteHost, tunnel.RemotePort)
	return nil
}